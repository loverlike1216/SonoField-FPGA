...
仓库根目录为 `E:\Codex-project\AMD-SonoField-FPGA`；运行目录为其 `v1` 子目录，已有板卡资料不要覆盖。

## 1. 确认版本与工作区

```powershell
Set-Location E:\Codex-project\AMD-SonoField-FPGA\v1
git remote -v
...
远端已有仓库由用户创建，本轮在同一仓库整理 v1 布局；没有自动创建新 GitHub 仓库。
...
target 路径相对仓库根目录，output 路径相对 v1。先提交源代码检查点，然后在全新、尚不存在的 build 子目录复现：
