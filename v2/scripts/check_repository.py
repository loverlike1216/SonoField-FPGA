...
def main():
    ...
    required=["README.md","AGENTS.md","CHANGELOG.md","shared/CURRENT_PLAN.md",
              ...
              "shared/report_v2.md","AI-chat-memory/INDEX.md","AI-chat-memory/SonoField-FPGA.md",
              "AI-chat-memory/CHAT_MEMORY_IMPORT_REQUIRED.md","AI-problem/README.md",
              "v2/README.md","v2/VERSION_MANIFEST.md","v2/docs/OPERATING_GUIDE.md",
              "AI-interaction-memory/INDEX.md","shared/versions/v1_freeze.json"]
    ...
    for p in documents:
        for link in re.findall(r"\]\(([^)]+)\)",p.read_text(encoding="utf-8")):
            if "://" not in link and not link.startswith("#") and not (p.parent/link.split("#")[0]).exists():
                ...
                # Historical root reports may link to a frozen version absent in sparse checkout.
                try: historical=target.relative_to(REPO).as_posix()
                except ValueError: historical=""
                exists_in_git=historical.startswith("v1/") and subprocess.run(
                    ["git","cat-file","-e","HEAD:"+historical],cwd=REPO,capture_output=True).returncode==0
                if not exists_in_git:errors.append(f"Broken link {p.relative_to(REPO)} -> {link}")
    state=json.loads((REPO/"shared/PROJECT_STATE.json").read_text(encoding="utf-8"))
    if state["active_version"]!="v2" or state["chat_source_name"]!="SonoField-FPGA":errors.append("State identity mismatch")
    ...
    result={"status":"PASS" if not errors else "FAIL","errors":errors,
            "checks":["v2 layout and frozen v1 integrity","complete inherited coverage","required artifacts","root hygiene","implementation markers",
